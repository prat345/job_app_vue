...
export const useJobsStore = defineStore("jobs", {
  ...
  actions: {
    async [FETCH_JOBS]() {
      ...
      this.jobs = jobs // update state
    }
  },
  // like computed property, property that derived(get) from other property
  getters: {
    // getter receive current state
    ...
    [INCLUDE_JOB_BY_ORGANIZATION]: () => (job) => {
      // return function ที่รับparameter job
    ...
    [FILTERED_JOBS](state) {
      return state.jobs
        .filter((job) => this.INCLUDE_JOB_BY_ORGANIZATION(job)) // run function ที่return ในgetter
